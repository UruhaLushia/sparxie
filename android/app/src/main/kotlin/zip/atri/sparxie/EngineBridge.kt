...
object EngineBridge {
    ...
    fun memoryInfo(): String {
        ...
        val javaCapacity = runtime.totalMemory()
        val javaFree = runtime.freeMemory()
        val status = runCatching {
            java.io.File("/proc/self/status").readLines().mapNotNull { line ->
                val parts = line.split(':', limit = 2)
                if (parts.size != 2) return@mapNotNull null
                val value = parts[1].trim().substringBefore(' ').toLongOrNull()
                    ?: return@mapNotNull null
                parts[0] to value * 1024L
            }.toMap()
        }.getOrDefault(emptyMap())
        fun bytes(key: String): Any = info.getMemoryStat(key)?.toLongOrNull()
            ?.takeIf { it >= 0 }?.let { it * 1024L } ?: JSONObject.NULL
        return JSONObject()
            .put("pss", info.totalPss.toLong() * 1024L)
            .put("private_dirty", info.totalPrivateDirty.toLong() * 1024L)
            .put("private_clean", info.totalPrivateClean.toLong() * 1024L)
            .put("shared_dirty", info.totalSharedDirty.toLong() * 1024L)
            .put("shared_clean", info.totalSharedClean.toLong() * 1024L)
            .put("rss", status["VmRSS"] ?: JSONObject.NULL)
            .put("peak_rss", status["VmHWM"] ?: JSONObject.NULL)
            .put("virtual_size", status["VmSize"] ?: JSONObject.NULL)
            .put("swap_pss", bytes("summary.total-swap-pss"))
            .put("java_heap", bytes("summary.java-heap"))
            .put("native_heap", bytes("summary.native-heap"))
            .put("graphics", bytes("summary.graphics"))
            .put("code", bytes("summary.code"))
            .put("stack", bytes("summary.stack"))
            .put("private_other", bytes("summary.private-other"))
            .put("system", bytes("summary.system"))
            .put("java_allocated", (javaCapacity - javaFree).coerceAtLeast(0L))
            .put("java_capacity", javaCapacity)
            .put("java_limit", runtime.maxMemory())
            .put("native_allocated", android.os.Debug.getNativeHeapAllocatedSize())
            .put("native_capacity", android.os.Debug.getNativeHeapSize())
            .put("native_free", android.os.Debug.getNativeHeapFreeSize())
            .toString()
    }

    @JvmStatic
    fun listApps(): String {
        val context = checkNotNull(appContext) { "Engine not initialized" }
        val pm = context.packageManager
        return JSONArray().apply {
            pm.getInstalledApplications(0).forEach { app ->
                if (app.enabled && app.packageName.isNotBlank()) {
                    ...
                    put(
                        JSONObject()
                            ...
                            .put("label", label)
                            .put("is_system", (app.flags and systemFlags) != 0)
                    )
                }
            }
        }.toString()
