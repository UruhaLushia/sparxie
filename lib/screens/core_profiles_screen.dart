...
class _CoreProfilesScreenState extends State<CoreProfilesScreen> {
  static const _configFiles = MethodChannel('zip.atri.sparxie/config_file');
  ...
  Future<void> _reload() async {
    try {
      ...
      if (!mounted) return;
      final previousOrder = _profiles.map((profile) => profile.id).toList();
      final byId = {for (final profile in profiles) profile.id: profile};
      final ordered = <rust.CoreConfigProfile>[
  ...
  Widget _profileList(BuildContext context) {
    ...
    return ReorderableListView.builder(
      ...
      onReorder: (oldIndex, newIndex) async {
        ...
        setState(() {});
        try {
          await rust.coreConfigReorder(
            ids: _profiles.map((p) => p.id).toList(),
          );
        } catch (_) {
          await _reload();
        }
        ...
        await showDialog<({String url, String text, String name, String ua})>(
          ...
          builder: (context) => StatefulBuilder(
            builder: (context, setDialogState) {
              ...
              final fileButtonStyle = CompactControlTheme.buttonOf(context)
                  .copyWith(foregroundColor: theme.colorScheme.primary);
              final cancelButtonStyle = CompactControlTheme.buttonOf(context)
                  .copyWith(backgroundColor: Colors.transparent);
                        ...
                        children: [
                          ...
                          CompactButton(
                            ...
                            onPressed: () async {
                              ...
                              try {
                                content =
                                    await _configFiles.invokeMethod<String>(
                                      'read',
                                      {'uri': file.path},
                                    ) ??
                                    '';
