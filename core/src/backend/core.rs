...
pub struct TunSettings {
    pub enabled: bool,
    /// "system" | "gvisor" | "mixed" | "mips" — kernel stack for the platform fd.
    pub stack: String,
    pub dns: String,
    pub ipv6: bool,
    pub mtu: u32,
...
pub async fn core_list_apps_window(
    query: String,
    offset: u32,
    limit: u32,
    show_system: bool,
) -> Result<AppWindow, MihomoError> {
    crate::core::list_apps_window(&query, offset, limit, show_system).await
}
...
pub async fn core_config_reorder(ids: Vec<String>) -> Result<(), MihomoError> {
    crate::core::config_reorder(ids).await
}

pub async fn core_config_set_mode(mode: String) -> Result<(), MihomoError> {
    crate::core::config_set_mode(&mode).await
}
