...
static APPS_CACHE: OnceLock<std::sync::Mutex<AppsCache>> = OnceLock::new();

pub(crate) fn list_apps_window(
    query: &str,
    offset: u32,
    limit: u32,
    show_system: bool,
) -> Result<AppWindow, MihomoError> {
    let cache = APPS_CACHE.get_or_init(|| std::sync::Mutex::new(AppsCache::default()));
    let mut guard = cache.lock().unwrap_or_else(|e| e.into_inner());
    let cache = &mut *guard;
    if cache.apps.is_none() {
        cache.apps = Some(list_all_apps()?);
    }
    let all = cache.apps.as_ref().expect("app cache populated");
    let filter = (query.trim().to_lowercase(), show_system);
    if cache.filter.as_ref() != Some(&filter) {
        cache.indices = all
            .iter()
            .enumerate()
            .filter(|(_, app)| {
                (show_system || !app.is_system)
                    && (app.search_label.contains(&filter.0)
                        || app.search_package.contains(&filter.0))
            })
            .map(|(index, _)| index)
            .collect();
        cache.filter = Some(filter);
    }
    let start = (offset as usize).min(cache.indices.len());
    let end = start
        .saturating_add(limit as usize)
        .min(cache.indices.len());
    Ok(AppWindow {
        apps: cache.indices[start..end]
            .iter()
            .map(|&index| all[index].info.clone())
            .collect(),
        total: cache.indices.len() as u32,
    })
}

pub(crate) fn memory_info() -> Result<crate::backend::api::memory::AppMemoryInfo, MihomoError> {
    with_vm(|env| {
        let json = env
            .call_static_method(get_bridge_class()?, "memoryInfo", "()Ljava/lang/String;", &[])?
            .l()?;
        let jstr = JString::from(json);
        let text = env.get_string(&jstr)?;
        serde_json::from_str(&text.to_string_lossy())
            .map_err(|error| MihomoError::InvalidJson(error.to_string()))
    })
}

fn list_all_apps() -> Result<Vec<InstalledApp>, MihomoError> {
    with_vm(|env| {
        let json = env
            .call_static_method(get_bridge_class()?, "listApps", "()Ljava/lang/String;", &[])?
            .l()?;
        if json.is_null() {
            return Err(MihomoError::Other("应用列表不可用".into()));
        }
        let jstr = JString::from(json);
        let text = env.get_string(&jstr)?;
        let mut apps: Vec<InstalledApp> = serde_json::from_str(&text.to_string_lossy())
            .map_err(|e| MihomoError::InvalidJson(e.to_string()))?;
        for app in &mut apps {
            app.search_label = app.info.label.to_lowercase();
            app.search_package = app.info.package.to_lowercase();
        }
        apps.sort_by(|a, b| {
            a.search_label
                .cmp(&b.search_label)
                .then_with(|| a.info.package.cmp(&b.info.package))
        });
        Ok(apps)
    })
}
