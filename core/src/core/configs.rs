...
pub(crate) fn reorder(ids: &[String]) -> Result<(), MihomoError> {
    let dir = profiles_dir()?;
    let mut index = load_index(&dir)?;
    ensure_builtin(&dir, &mut index)?;
    let mut ordered = Vec::with_capacity(index.profiles.len());
    for id in ids {
        if let Some(pos) = index.profiles.iter().position(|p| &p.id == id) {
            ordered.push(index.profiles.remove(pos));
        }
    }
    ordered.append(&mut index.profiles);
    index.profiles = ordered;
    save_index(&dir, &index)
}
