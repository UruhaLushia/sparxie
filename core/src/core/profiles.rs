...
pub(crate) async fn config_reorder(ids: Vec<String>) -> Result<(), MihomoError> {
    #[cfg(target_os = "android")]
    {
        let _profiles = super::core().profile_operation.lock().await;
        return tokio::task::spawn_blocking(move || configs::reorder(&ids))
            .await
            .map_err(|error| MihomoError::Other(format!("排序配置任务失败:{error}")))?;
    }
    #[cfg(not(target_os = "android"))]
    {
        let _ = ids;
...
pub(crate) async fn config_set_mode(mode: &str) -> Result<(), MihomoError> {
    #[cfg(target_os = "android")]
    {
        let mode = mode.trim().to_ascii_lowercase();
        if !matches!(mode.as_str(), "rule" | "global" | "direct") {
            return Err(MihomoError::Other("出站模式无效".into()));
        }
        let _operation = super::core().operation.lock().await;
        return tokio::task::spawn_blocking(move || {
            super::go_bridge::patch_config(&format!(r#"{{"mode":"{mode}"}}"#))
        })
        .await
        .map_err(|error| MihomoError::Other(format!("切换出站模式任务失败:{error}")))?;
    }
    #[cfg(not(target_os = "android"))]
    {
        let _ = mode;
...
#[cfg(target_os = "android")]
pub(crate) async fn list_apps_window(
    query: &str,
    offset: u32,
    limit: u32,
    show_system: bool,
    ...
    tokio::task::spawn_blocking(move || {
        android::list_apps_window(&query, offset, limit, show_system)
    })
    .await
    .map_err(|error| MihomoError::Other(format!("读取应用列表失败:{error}")))?
}
